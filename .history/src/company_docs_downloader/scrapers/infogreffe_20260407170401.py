...
class InfogreffeClient(BaseScraper):
    ...
    def _open_company_page(self, page: Page, query: CompanyQuery, company: CompanyIdentity | None = None) -> None:
        ...
        self._open_company_result(page, result_link)
    ...
    def _open_documents_tab(self, page: Page) -> None:
        documents_tab = self._wait_for_any(
            [
                ...
                page.get_by_role("button", name=re.compile(r"Documents", re.I)),
                page.locator("a, button, [role='tab'], [role='button']").filter(has_text=re.compile(r"Documents", re.I)),
                page.get_by_text(re.compile(r"Documents", re.I)),
            ],
            "onglet Documents Infogreffe",
        )
        self._robust_click(page, documents_tab, "onglet Documents Infogreffe")
    ...
    def _open_beneficial_owners_tab(self, page: Page) -> None:
        ...
        tab = self._wait_for_any(
            [
                ...
                page.get_by_role("button", name=re.compile(r"B[ée]n[ée]ficiaires? effectifs?", re.I)),
                page.locator("a, button, [role='tab'], [role='button']").filter(
                    has_text=re.compile(r"B[ée]n[ée]ficiaires? effectifs?|b[ée]n[ée]ficiaire", re.I)
                ),
        ...
        self._robust_click(page, tab, "onglet Beneficiaires effectifs Infogreffe")
    ...
    def _find_rbe_download(self, page: Page):
        direct_candidates = [
            ...
            page.locator("a, button, [role='button'], [role='link']").filter(
                has_text=re.compile(r"T[ée]l[ée]charger la copie int[ée]grale pour les assujettis", re.I)
            ),
            page.get_by_role("button", name=re.compile(r"copie int[ée]grale.*assujettis", re.I)),
            page.get_by_role("link", name=re.compile(r"copie int[ée]grale.*assujettis", re.I)),
        ]

        direct_target = self._find_visible_locator_in_scopes(
            page,
            [lambda _scope, locator=locator: locator for locator in direct_candidates],
        )
    ...
    def _is_company_profile_page(self, page: Page) -> bool:
        if re.search(r"(entreprise|societe|documents|etablissements|dirigeants|analyse-financiere|beneficiaires-effectifs|beneficiaires_effectifs|fiche)", page.url, re.I):
            return True

        profile_markers = [
            lambda scope: scope.get_by_role("tab", name=re.compile(r"Identit[ée]|Documents|Dirigeants|Etablissements|Analyse financi[èe]re|B[ée]n[ée]ficiaires? effectifs?", re.I)),
            lambda scope: scope.get_by_role("link", name=re.compile(r"Identit[ée]|Documents|Dirigeants|Etablissements|Analyse financi[èe]re|B[ée]n[ée]ficiaires? effectifs?", re.I)),
            lambda scope: scope.get_by_text(re.compile(r"Identit[ée]|Documents|Dirigeants|Etablissements|Analyse financi[èe]re|B[ée]n[ée]ficiaires? effectifs?", re.I)),
        ]
    ...
    def _find_company_result(
        ...
        simple_candidates = [
            page.locator("a[href*='entreprise']").filter(has_text=re.compile(re.escape(search_value), re.I)),
            page.locator("a[href*='societe']").filter(has_text=re.compile(re.escape(search_value), re.I)),
            page.locator("a[href*='fiche']").filter(has_text=re.compile(re.escape(search_value), re.I)),
            page.get_by_role("link").filter(has_text=re.compile(re.escape(search_value), re.I)),
            page.get_by_role("link").filter(has_text=re.compile(re.escape(query.value), re.I)),
        ]
        if company and company.name:
            simple_candidates.append(page.get_by_role("link").filter(has_text=re.compile(re.escape(company.name), re.I)))

        try:
            return self._wait_for_any(simple_candidates, "resultat entreprise Infogreffe")
    ...
    def _open_company_result(self, page: Page, result_link: Locator) -> None:
        href = None
        try:
        ...
        self._robust_click(page, result_link, "resultat entreprise Infogreffe")
    ...
    def _robust_click(self, page: Page, locator: Locator, description: str) -> None:
        ...
        for target in click_targets:
            try:
                target.wait_for(state="visible", timeout=600)
