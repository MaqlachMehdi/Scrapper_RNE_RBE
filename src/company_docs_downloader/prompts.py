...
def _ask_infogreffe_credentials(*, allow_save: bool) -> Credentials:
    ...
    if allow_save:
        wants_save = questionary.confirm(
            "Enregistrer ces identifiants dans le gestionnaire d'identifiants Windows pour ne plus les ressaisir ?",
            default=True,
        ).ask()
        if wants_save:
