...
class InfogreffeClient(BaseScraper):
    ...
    def _open_beneficial_owners_tab(self, page: Page) -> None:
        initial_target = self._find_visible_locator_in_scopes(page, self._rbe_download_locator_builders(page))
        if initial_target is not None:
            self._align_target_in_view(page, initial_target)
            return

        section_candidates = [
            page.locator("#beneficiaires_effectifs"),
            page.locator("#beneficiaires-effectifs"),
            page.locator("[id*='beneficiaires']"),
            page.locator("[class*='beneficiaires']"),
            page.get_by_text(re.compile(r"B[ée]n[ée]ficiaires? effectifs?", re.I)),
        ]

        for candidate in section_candidates:
            try:
                candidate.first.scroll_into_view_if_needed(timeout=self.timeout_ms)
                page.wait_for_timeout(300)
                target = self._find_visible_locator_in_scopes(page, self._rbe_download_locator_builders(page))
